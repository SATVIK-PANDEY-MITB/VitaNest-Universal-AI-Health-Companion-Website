    ...
    try {
      ...
      if (userError) {
        ...
        if (userError.code === 'PGRST116') {
          console.log('⚠️ User profile not found, returning default profile');
          return {
            id: userId,
            email: '',
            name: 'User',
      ...
      // Then get the health profile separately
      const { data: healthData, error: healthError } = await supabase
        .from('health_profiles')
        .select('*')
        .eq('user_id', userId)
        .maybeSingle(); // Use maybeSingle to handle case where no health profile exists

      if (healthError) {
        console.warn('⚠️ Health profile fetch error (non-critical):', healthError);
      }
  ...
  static async updateUserProfile(userId: string, updates: any) {
    try {
      ...
      if (updates.subscription !== undefined) {
        if (!['free', 'premium', 'enterprise'].includes(updates.subscription)) {
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          ...
          throw new Error(`Failed to update user profile: ${error.message || 'Unknown database error'}`);
        }
      }

      if (!data) {
        throw new Error('Update completed but no data returned. Please refresh the page.');
      }
